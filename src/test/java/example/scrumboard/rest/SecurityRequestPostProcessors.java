...
/**
 * Demonstrates how to use a {@link RequestPostProcessor} to add request-building methods for establishing a security
 * context for Spring Security. While these are just examples, <a
 * href="https://jira.springsource.org/browse/SEC-2015">official support</a> for Spring Security is planned.
 * 
 * @author Rob Winch
 */
public class SecurityRequestPostProcessors {
	...
	/**
	 * Establish a security context for a user with the specified username. All details are declarative and do not
	 * require that the user actually exists. This means that the authorities or roles need to be specified too.
	 */
	...
	/**
	 * Establish a security context for a user with the specified username. The additional details are obtained from the
	 * {@link UserDetailsService} declared in the {@link WebApplicationContext}.
	 */
	...
	/**
	 * Establish a security context with the given {@link SecurityContext} and thus be authenticated with
	...
	public final static class SecurityContextRequestPostProcessor extends SecurityContextRequestPostProcessorSupport
	...
	public final static class UserRequestPostProcessor extends SecurityContextRequestPostProcessorSupport implements
		...
		/**
		 * Sets the prefix to append to each role if the role does not already start with the prefix. If no prefix is
		 * desired, an empty String or null can be used.
		 */
		 ...
		 * 
		 * @param roles
		 *            The roles to populate. Note that if the role does not start with {@link #rolePrefix(String)} it
		 *            will automatically be prepended. This means by default {@code roles("ROLE_USER")} and
		...
		@Override
		public MockHttpServletRequest postProcessRequest(MockHttpServletRequest request) {
			UsernamePasswordAuthenticationToken authentication = new UsernamePasswordAuthenticationToken(this.username,
					this.credentials, this.authorities);
			save(authentication, request);
			return request;
		}
	}

	public final static class UserDetailsRequestPostProcessor extends SecurityContextRequestPostProcessorSupport
			implements RequestPostProcessor {
		...
		/**
		 * Use this method to specify the bean id of the {@link UserDetailsService} to use to look up the
		 * {@link UserDetails}.
		 * 
		 * <p>
		 * By default a lookup of {@link UserDetailsService} is performed by type. This can be problematic if multiple
		 * {@link UserDetailsService} beans are declared.
		 */
